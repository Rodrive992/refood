...
@php
    // ✅ Hora Argentina (por si en controller no la pasaste)
    ...
    /**
     * ✅ Items: usar los campos reales que se ven en tu _poll:
     * - nombre_snapshot
     * - cantidad
     * - nota
     */
...
<div style="margin-top: 4px;"><b>Mozo:</b> {{ $mozoNombre }}</div>
...
<div class="items">
    @forelse($items as $it)
        ...
        <div class="item">
            ...
            @if($nota !== '')
                <div class="muted" style="margin-top: 3px;">- {{ $nota }}</div>
            @endif
        </div>
    @empty
...
<div class="hr"></div>
<div class="center muted">— FIN —</div>

<script>
(function(){
    function notifyParent(){
        try {
            window.parent && window.parent.postMessage({ type: 'RF_PRINT_DONE', comanda_id: {{ (int)$comanda->id }} }, '*');
        } catch(e){}
    }
    ...
    window.addEventListener('load', function(){
        setTimeout(function(){
            ...
            // algunos navegadores no disparan afterprint bien en iframe
            notifyParent();
        }, 80);
    });
